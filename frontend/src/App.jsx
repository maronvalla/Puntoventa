...
export default function App() {
  ...
  const [employeeStockSearch, setEmployeeStockSearch] = useState("");
  ...
  async function addEmployeeStock(product) {
    if (isAdmin || Number(product.costPrice) !== 0) return;
    const quantity = Number(stockAdjust[product.id]?.delta);
    if (!Number.isInteger(quantity) || quantity <= 0) {
      return feedback("Ingresá una cantidad entera mayor a cero.", "error");
    }

    setStockSubmittingId(product.id);
    try {
      await api.adjustStock(product.id, quantity, "Carga desde portal de empleado");
      setStockAdjust((current) => ({ ...current, [product.id]: { delta: "" } }));
      await loadData();
      feedback(`Se sumaron ${quantity} unidades a ${product.name}.`);
    } catch (error) {
      feedback(error.message || "No se pudo cargar el stock.", "error");
    } finally {
  ...
  const employeeStockProducts = useMemo(() => {
    const query = employeeStockSearch.trim().toLowerCase();
    return products
      .filter((product) => product.active && Number(product.costPrice) === 0)
      .filter((product) => !query || [product.name, product.code, product.barcode]
        .some((value) => String(value || "").toLowerCase().includes(query)))
      .sort((a, b) => String(a.name).localeCompare(String(b.name), "es"));
  }, [products, employeeStockSearch]);
  ...
  return (
    <div className={`min-h-screen bg-slate-50 text-slate-900 flex ${isAdmin ? "admin-shell" : ""}`}>
      ...
      <aside className={isAdmin ? "hidden" : "w-72 bg-slate-900 text-slate-100 p-6 flex flex-col gap-2 border-r border-slate-800 shadow-lg"}>
        ...
        {!isAdmin && (
          <button
            ...
            className={
              "px-4 py-3 rounded-lg text-left transition " +
              (view === "stock-entry" ? "bg-blue-600 text-white shadow" : "text-slate-300 hover:bg-slate-800")
            }
          >
            Cargar stock
          </button>
      ...
      <main className={`min-w-0 flex-1 overflow-y-auto ${isAdmin ? "admin-main p-4 sm:p-6 lg:p-8" : "p-6 lg:p-8"}`}>
        ...
        {view === "stock-entry" && !isAdmin && (
          <div className="mx-auto max-w-4xl space-y-5">
            <section className="rounded-2xl border border-blue-100 bg-blue-50 p-5">
              <h3 className="text-lg font-bold text-blue-950">Carga rápida de stock</h3>
              <p className="mt-1 text-sm text-blue-800">Sólo podés sumar unidades a productos configurados con costo $0. Cada carga queda registrada con tu usuario.</p>
            </section>

            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <label className="block text-sm font-semibold text-slate-700" htmlFor="employee-stock-search">Buscar producto</label>
              <input
                id="employee-stock-search"
                className="mt-2 w-full rounded-xl border border-slate-200 p-3 focus:outline-none focus:ring-2 focus:ring-blue-500/30"
                placeholder="Nombre, código o código de barras"
                value={employeeStockSearch}
                onChange={(event) => setEmployeeStockSearch(event.target.value)}
              />
            </div>

            <div className="grid gap-4 sm:grid-cols-2">
              {employeeStockProducts.map((product) => (
                <article key={product.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div><h3 className="font-bold text-slate-900">{product.name}</h3><p className="mt-1 text-xs text-slate-500">{product.code || "Sin código"}</p></div>
                    <span className="rounded-full bg-slate-100 px-3 py-1 text-sm font-bold text-slate-700">{product.stock} u.</span>
                  </div>
                  <label className="mt-5 block text-sm font-semibold text-slate-700" htmlFor={`employee-stock-${product.id}`}>Unidades a sumar</label>
                  <div className="mt-2 flex gap-2">
                    <input
                      id={`employee-stock-${product.id}`}
                      className="min-w-0 flex-1 rounded-xl border border-slate-200 p-3"
                      type="number"
                      min="1"
                      step="1"
                      inputMode="numeric"
                      value={stockAdjust[product.id]?.delta ?? ""}
                      onChange={(event) => setStockAdjust((current) => ({ ...current, [product.id]: { delta: event.target.value } }))}
                      placeholder="Ej. 10"
                    />
                    <button
                      className="rounded-xl bg-blue-600 px-4 font-bold text-white transition hover:bg-blue-700 disabled:bg-slate-300"
                      disabled={stockSubmittingId === product.id}
                      onClick={() => addEmployeeStock(product)}
                    >
                      {stockSubmittingId === product.id ? "Guardando..." : "Sumar"}
                    </button>
                  </div>
                </article>
              ))}
            ...
            {!employeeStockProducts.length && <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center text-slate-500">No hay productos de costo $0 que coincidan con la búsqueda.</div>}
          </div>
        )}
