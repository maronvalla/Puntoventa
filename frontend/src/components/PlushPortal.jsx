...
function Dialog({ title, open, onClose, children, submitLabel = "Guardar", onSubmit, wide = false }) {
  if (!open) return null;
  return <div className="fixed inset-0 z-[70] grid place-items-center p-4">
    ...
    <form role="dialog" aria-modal="true" aria-label={title} className={`relative max-h-[92vh] w-full overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl ${wide ? "max-w-3xl" : "max-w-lg"}`} onSubmit={(event) => { event.preventDefault(); onSubmit(); }}>
      ...
      {children}
      <div className="mt-6 flex justify-end gap-3"><button type="button" className="btn btn-ghost" onClick={onClose}>Cancelar</button><button className="btn btn-primary" type="submit">{submitLabel}</button></div>
    </form>
  </div>;
}
...
export default function PlushPortal({ api, notify }) {
  ...
  const [settlementForm, setSettlementForm] = useState({ machineId: "", finalCounter: "", cashAmount: "", qrAmount: "", notes: "", id: null });
  ...
  function openSettlement(machine) {
    const latest = (data?.settlements || []).find((item) => item.machineId === machine.id && item.status === "ACTIVE");
    setSettlementForm({ machineId: machine.id, finalCounter: String(latest?.finalCounter ?? machine.initialCounter), cashAmount: "", qrAmount: "", notes: "", id: null });
    setDialog("settlement");
  }

  function openSettlementEdit(item) {
    setSettlementForm({ machineId: item.machineId, finalCounter: String(item.finalCounter), cashAmount: String(item.cashAmount), qrAmount: String(item.qrAmount), notes: item.notes || "", id: item.id });
    setDialog("settlement");
  }
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    {tab === "machines" && selectedMachine && <>
      <div className="flex flex-wrap items-center justify-between gap-3"><button className="btn btn-ghost" onClick={() => setSelectedMachineId(null)}>← Todas las máquinas</button><div className="flex flex-wrap gap-2"><button className="btn btn-ghost" onClick={() => openMachine(selectedMachine)}>Editar ficha</button><button className="btn btn-soft" onClick={() => openLoad(selectedMachine.id)}>Cargar peluches</button><button className="btn btn-primary" onClick={() => openSettlement(selectedMachine)}>Nueva liquidación</button></div></div>
      ...
      <section className="grid gap-6 xl:grid-cols-2"><div className="admin-card"><div className="card-heading"><div><h3>Liquidaciones</h3><p>Contadores y rentabilidad histórica</p></div></div><div className="space-y-3">{machineSettlements.map((item) => <article key={item.id} className={`rounded-xl border p-4 ${item.status === "VOIDED" ? "border-slate-200 bg-slate-50 opacity-65" : "border-slate-200"}`}><div className="flex justify-between gap-3"><div><b>{dateTime(item.createdAt)}</b><div className="mt-1 text-xs text-slate-500">Contador {item.initialCounter} → {item.finalCounter} · {item.prizesDelivered} premios</div></div><div className="text-right"><b>{ars(item.grossIncome)}</b><div className={`text-xs ${item.netProfit >= 0 ? "text-emerald-600" : "text-rose-600"}`}>Neto {ars(item.netProfit)}</div></div></div>{item.status === "ACTIVE" && latestActiveSettlement?.id === item.id && <div className="mt-3 flex gap-2 border-t pt-3"><button className="text-button" onClick={() => openSettlementEdit(item)}>Corregir</button><button className="text-button text-rose-600" onClick={() => reasonAndRun("liquidación", (reason) => api.voidPlushSettlement(item.id, reason))}>Anular</button></div>}{item.status === "VOIDED" && <div className="mt-2 text-xs text-rose-600">Anulada: {item.voidReason}</div>}</article>)}{!machineSettlements.length && <div className="empty-state">Sin liquidaciones.</div>}</div></div>
    ...
    <Dialog open={dialog === "settlement"} title={settlementForm.id ? "Corregir última liquidación" : "Nueva liquidación"} onClose={() => setDialog(null)} submitLabel={busy ? "Guardando…" : "Guardar liquidación"} onSubmit={() => run(() => settlementForm.id ? api.updatePlushSettlement(settlementForm.id, settlementForm) : api.createPlushSettlement(settlementForm), "Liquidación guardada.")}><div className="space-y-4"><div className="rounded-xl bg-fuchsia-50 p-4 text-sm"><div className="text-fuchsia-700">Contador inicial automático</div><b className="text-xl">{settlementForm.id ? latestActiveSettlement?.initialCounter : ((data?.settlements || []).find((item) => item.machineId === settlementForm.machineId && item.status === "ACTIVE")?.finalCounter ?? data?.machines?.find((item) => item.id === settlementForm.machineId)?.initialCounter ?? "—")}</b></div><Field label="Lectura final"><input autoFocus required className="admin-input" type="number" min="0" value={settlementForm.finalCounter} onChange={(e) => setSettlementForm({ ...settlementForm, finalCounter: e.target.value })}/></Field><div className="grid grid-cols-2 gap-4"><Field label="Efectivo"><input className="admin-input" type="number" min="0" step="0.01" value={settlementForm.cashAmount} onChange={(e) => setSettlementForm({ ...settlementForm, cashAmount: e.target.value })}/></Field><Field label="QR"><input className="admin-input" type="number" min="0" step="0.01" value={settlementForm.qrAmount} onChange={(e) => setSettlementForm({ ...settlementForm, qrAmount: e.target.value })}/></Field></div><Field label="Notas"><textarea className="admin-input" value={settlementForm.notes} onChange={(e) => setSettlementForm({ ...settlementForm, notes: e.target.value })}/></Field></div></Dialog>
  </div>;
}
